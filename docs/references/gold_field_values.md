...
| `class_of_ms` | `Option<ClassOfMs>` (24 bit) | 24 (when present) | **`Some(0x1A1060)`** — bit-decoded 2026-05-03 aus Gold UL#0+UL#1 Reassembly. `reference_gold_full_attach_timeline.md` Z.76 zitiert `0x1A0F60` (Tippfehler im Memo); maßgeblich ist der Bit-Decode `0x1A1060`. | UL#0+UL#1 reassembled, 6× konsistent | decode_dl/ul-Lauf 2026-05-03; bit-decode siehe ClassOfMs in bluestation `mm/fields/class_of_ms.rs` |
