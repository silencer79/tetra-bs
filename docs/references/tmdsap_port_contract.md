...
|---|---|---|---|---|---|
| `0x170` | `REG_TMDSAP_TX_FRAMES_CNT` | `[31:0]` | `tetra_tmdsap_tx_framer.tlm_tx_frames` | `0` | Saturating count of successfully decoded TMDC frames handed off to LMAC TX. |
| `0x174` | `REG_TMDSAP_RX_FRAMES_CNT` | `[31:0]` | `tetra_tmdsap_rx_framer.tlm_rx_frames` | `0` | Saturating count of TMDC frames emitted on AXIS-master to A1's S2MM port. |
| `0x178` | `REG_TMDSAP_ERR_CNT`       | `[31:0]` | `tetra_tmdsap_tx_framer.tlm_err_count` | `0` | Saturating count of frame-error events (bad magic, bad length, bad tkeep, premature/missing tlast). |

**Offset rationale.**  Merge-time reconcile (2026-05-03): A2 TmaSap
counters claim `0x160..0x16C` (RX_FRAMES, TX_FRAMES, TX_ERR, TMAR_FRAMES),
A3 TmdSap counters land directly above at `0x170/0x174/0x178`. Both
within the explicitly-reserved "future per-layer counters" band
`0x160..0x1FF`. Clear of A1 (`0x120..0x13F`) and AACH/UMAC counters
(`0x140..0x15F`).
